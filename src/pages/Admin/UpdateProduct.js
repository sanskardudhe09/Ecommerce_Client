...
const UpdateProduct = () => {
    ...
    const getSingleProd = async () => {
        try {
            const { data } = await axios.get(`https://ecommerce-server-6dot.onrender.com/api/get-product/${params.slug}`);
            if (data.product) {
    ...
    const handleUpdateProd = async (e) => {
        ...
        try {
            ...
            const { data } = await axios.put(`https://ecommerce-server-6dot.onrender.com/api/update-product/${id}`, prodData);
            console.log(data);
            if (data?.product) {
    ...
    const handleDeleteProd = async () => {
        try {
            ...
            if(!ans) return;
            const {data} = await axios.delete(`https://ecommerce-server-6dot.onrender.com/api/delete-product/${id}`);
            if(data.product){
                navigate(`/dashboard/admin/products`)
                setTimeout(()=>{
                        ...
                        <div className='w-75 m-3'>
                            ...
                            <div className='mb-3'>
                                {photo ? (
                                    ...
                                    <div className='text-center'>
                                        <img src={ `https://ecommerce-server-6dot.onrender.com/api/product-photo/${id}`}
                                            alt="product-photo" height={"200px"}
                                            className='img img-responsive' />
                                    </div>
                                )}
