...
const HomePage = () => {
  ...
  const getAllProducts = async () => {
    try {
      setLoading(true);
      const { data } = await axios.get(`https://ecommerce-server-pearl.vercel.app/api/product-list/${page}`);
      setLoading(false);
      if (data?.products) {
  ...
  const getProdCount = async () => {
    try {
      const { data } = await axios.get(`https://ecommerce-server-pearl.vercel.app/api/product-count`);
      setTotal(data?.count);
    } catch (error) {
      toast.error("Something went wrong!!")
    }
  ...
  const filterProducts = async (req, res) => {
    try {
      const { data } = await axios.post(`https://ecommerce-server-pearl.vercel.app/api/product-filters`, { checked, radio });
      if (data?.product) {
  ...
  return (
    <Layout>
      <div className='container-fluid row mt-5 pe-5 homepage'>
        ...
        <div className='col-md-10 products'>
          ...
          <div className='d-flex flex-wrap'>
            {products?.map((p) => (
              <div className="card m-3 product-card" style={{ width: '28rem', borderColor: "lightblue" }} key={p._id}>
