...
const CartPage = () => {
    ...
    const getTokens = async () => {
        try {
            const {data} = await axios.get(`https://ecommerce-server-6dot.onrender.com/api/braintree-token`);
            console.log(data);
    ...
    const handlePayment = async () => {
        try {
            ...
            const {data} = await axios.post(`https://ecommerce-server-6dot.onrender.com/api/payment`, {nonce, cart});
            setLoading(false);
    ...
    return (
        <Layout>
            <div>
                ...
                <div className='row mt-3 ms-5 cart-page'>
